...
class TestRepayRecord(TestCase):
    def setUp(self):
        self.client = Client()
        ...
        status_code, response_dict = customer_loan(self.bank_teller_token, self.customer_id, 100, 30, "2021-3-20")
    ...
    def test_query_deposits_by_customer_id(self):
        # 正确查询用户存款记录测试
        status_code, response_dict = query_repays_by_customer_id(self.bank_teller_token, self.customer_id)
        self.assertEqual(200, status_code)
        self.assertEqual(1, len(response_dict))

        # 无柜员权限测试
        status_code, response_dict = query_repays_by_customer_id('wrong token', self.customer_id)
        self.assertEqual(401, status_code)
        self.assertEqual("Unauthorized", response_dict)

        # 参数缺失测试
        status_code, response_dict = query_repays_by_customer_id(self.bank_teller_token)
