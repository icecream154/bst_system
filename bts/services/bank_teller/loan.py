...
def loan(request):
    ...
    new_loan_record = LoanRecord(customer_id=customer_id, payment=payment,
...
def query_loan_record_by_customer_id(request):
    ...
    customer_load_record_list = list(LoanRecord.objects.filter(customer_id=customer_id))
    response_data = []
...
def _loan_repay(loan_record: LoanRecord, repay):
    new_loan_repay = LoanRepay(loan_record_id=loan_record.loan_record_id,
...
def auto_repay_process(request):
    ...
    for loan_record in load_record_query_set:
        ...
        if loan_record.left_payment > 0 and datetime.now() >= loan_record.due_date:
            customer = Customer.objects.get(customer_id=loan_record.customer_id)
            curr_repay = 0
            left_payment_before = loan_record.left_payment
            left_fine_before = loan_record.left_fine
